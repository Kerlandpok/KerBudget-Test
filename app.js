...
const D=window.INITIAL_DATA, KEY='budget2026.test.v2', BACKUP_KEY='budget2026.test.backups', APP_VERSION='2.5.0-test', MAX_BACKUPS=12;
...
function home(){const t=totals(month),p=planned(month),current=balance(month,true),projected=balance(month,false),savings=savingsBalance(),savedThisMonth=monthSavings(month),pendingBills=billsPending(month),pendingAll=unpointedMovements(month),days=daysLeftInMonth(month),remaining=Math.max(0,p.expense-t.expense),daily=remaining/days,goal=savingsGoal(),progress=goal>0?Math.min(100,Math.max(0,savings/goal*100)):0,status=dashboardStatus(month);let cat={};for(const x of mtx(month).filter(x=>isExpenseType(x)&&x.pointed))cat[x.category]=(cat[x.category]||0)+x.amount;const cats=Object.entries(cat).sort((a,b)=>b[1]-a[1]).slice(0,5);return layout(`${monthbar()}<section class="status-banner ${status.tone}"><span>${status.icon}</span><strong>${status.text}</strong></section><div class="dashboard-grid"><section class="dash-card primary"><div class="dash-label">💳 Compte courant</div><div class="dash-value">${euro(current)}</div><div class="dash-sub">Prévision fin de mois : <b class="${projected>=0?'positive':'negative'}">${euro(projected)}</b></div></section><section class="dash-card"><div class="dash-label">🏦 Livret A</div><div class="dash-value">${euro(savings)}</div><div class="dash-sub">Épargné ce mois : <b>${euro(savedThisMonth)}</b></div></section><section class="dash-card"><div class="dash-label">💶 Disponible ce mois</div><div class="dash-value">${euro(remaining)}</div><div class="dash-sub">${euro(daily)} par jour sur ${days} jour${days>1?'s':''}</div></section><section class="dash-card"><div class="dash-label">📅 À faire</div><div class="dash-value">${pendingAll.length}</div><div class="dash-sub">${pendingBills.length} facture${pendingBills.length>1?'s':''} à pointer</div></section></div>${pendingAll.length?`<div class="section-title"><h2>À pointer</h2><button class="btn secondary" data-go="transactions">Tout voir</button></div>${txList(pendingAll.slice(0,5))}`:''}<div class="section-title"><h2>Ce mois-ci</h2></div><div class="summary-grid"><div class="summary-tile"><span>Revenus</span><b class="positive">${euro(t.income)}</b></div><div class="summary-tile"><span>Dépenses</span><b class="negative">${euro(t.expense)}</b></div><div class="summary-tile"><span>Épargne</span><b>${euro(savedThisMonth)}</b></div><div class="summary-tile"><span>Budget prévu</span><b>${euro(p.expense)}</b></div></div><div class="section-title"><h2>Objectif d’épargne</h2><button class="btn secondary" data-go="savings">Détails</button></div><section class="goal-card"><div class="goal-top"><span>${euro(savings)} sur ${euro(goal)}</span><b>${progress.toFixed(0)} %</b></div><div class="progress large"><i style="width:${progress}%"></i></div><div class="dash-sub">Reste à épargner : ${euro(Math.max(0,goal-savings))}</div></section><div class="section-title"><h2>Dépenses principales</h2><button class="btn secondary" data-go="budget">Budget</button></div><div class="list">${cats.length?cats.map(([c,v])=>`<div class="row"><div><div class="title">${esc(c)}</div><div class="progress"><i style="width:${Math.min(100,v/(t.expense||1)*100)}%"></i></div></div><div class="amount">${euro(v)}</div></div>`).join(''):'<div class="empty">Aucune dépense pointée.</div>'}</div><div class="section-title"><h2>Derniers mouvements</h2><button class="btn secondary" data-go="transactions">Tout voir</button></div>${txList(mtx(month).slice(-6).reverse())}<button class="fab" data-add>+</button>`) }
...
function ensureRecurringForMonth(m,notify=false){
  ...
  for(const r of state.recurringBills||[]){
    ...
    state.transactions.push(normalizeMovement({
      ...
      month:m,day:Math.min(31,Math.max(1,Number(r.day)||1)),
