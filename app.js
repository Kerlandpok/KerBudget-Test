...
const D=window.INITIAL_DATA, KEY='budget2026.test.v2', BACKUP_KEY='budget2026.test.backups', APP_VERSION='3.7.7-test', BASE_YEAR=2026, MAX_BACKUPS=5;
...
function normalizeLoanScheduleInPlace(loan){
  if(!loan||!Array.isArray(loan.schedule))return;
  const seen=new Set();
  loan.schedule=loan.schedule.map((inst,i)=>({
    ...inst,
    id:String(inst.id||`${loan.id}-i${i+1}`),
  ...
  }).sort((a,b)=>(parseDate(a.date)||new Date(8640000000000000))-(parseDate(b.date)||new Date(8640000000000000)));
  let remaining=Math.max(0,Number(loan.totalDue)||loan.schedule.reduce((a,x)=>a+(Number(x.amount)||0),0));
  loan.schedule.forEach(inst=>{remaining=Math.max(0,remaining-(Number(inst.capital)||Number(inst.amount)||0));inst.remaining=+remaining.toFixed(2)});
  loan.installmentCount=loan.schedule.length;
  if(loan.schedule.length){loan.startDate=loan.startDate||loan.schedule[0].date;loan.endDate=loan.schedule.at(-1).date}
}
function loanStats(loan){normalizeLoanScheduleInPlace(loan);const schedule=loan.schedule||[],idx=buildLoanPointIndex(),today=new Date();today.setHours(0,0,0,0);const total=Number(loan.totalDue)||schedule.reduce((a,x)=>a+(Number(x.amount)||0),0);const paidRows=schedule.filter(x=>loanInstallmentPointed(loan,x,idx));const paid=paidRows.reduce((a,x)=>a+(Number(x.amount)||0),0);const remaining=Math.max(0,total-paid);const future=schedule.filter(x=>(parseDate(x.date)||new Date(0))>=today&&!loanInstallmentPointed(loan,x,idx));const end=parseDate(loan.endDate||schedule.at(-1)?.date),months=end?Math.max(0,(end.getFullYear()-today.getFullYear())*12+end.getMonth()-today.getMonth()):0;return{total,paid,remaining,progress:total?Math.min(100,paid/total*100):0,next:future[0]||null,months,years:Math.floor(months/12),extraMonths:months%12,paidCount:paidRows.length,remainingCount:Math.max(0,schedule.length-paidRows.length)}}
...
function upcomingWithinDays(i,days=7,forecast=null){const from=currentDayForMonth(i),to=Math.min(new Date(selectedYear,i+1,0).getDate(),from+days-1);const f=forecast||forecastData(i);return f.rows.filter(x=>x.day>=from&&x.day<=to)}
function smartInsights(i,forecast=null){
  const out=[], f=forecast||forecastData(i);
...
function home(){
  const current=balance(month,true), f=forecastData(month), savingsTotal=savingsBalance(), pending=unpointedMovements(month), week=upcomingWithinDays(month,7,f), weekOut=week.filter(x=>x.amount<0).reduce((s,x)=>s+Math.abs(x.amount),0), upcoming=f.rows.slice(0,5), insights=smartInsights(month,f), t=totals(month), p=planned(month), budgetInfo=budgetState(p.expense,t.expense,false), budgetUsed=Math.min(100,budgetInfo.pct);
...
function loanDetail(){const loan=loanById(activeLoanId);if(!loan){view='loans';return loansPage()}const x=loanStats(loan),rows=loan.schedule||[];return layout(`<div class="section-title"><button class="back-link" data-go="loans">‹ Prêts</button><div class="section-actions"><button class="btn secondary" data-loan-edit="${esc(loan.id)}">Modifier</button><button class="btn danger" data-loan-delete="${esc(loan.id)}">Supprimer</button></div></div>
  ...
  <section class="loan-progress-card"><div><span>Remboursé</span><b>${euro(x.paid)} / ${euro(x.total)}</b></div><div class="progress"><i style="width:${x.progress}%"></i></div><small>${x.progress.toFixed(1)} % · ${x.paidCount} échéance${x.paidCount>1?'s':''} réglée${x.paidCount>1?'s':''} · ${x.remainingCount} restante${x.remainingCount>1?'s':''}${x.next?` · prochaine le ${parseDate(x.next.date)?.toLocaleDateString('fr-FR')}`:''}</small></section>
...
function forecastData(i){
  ...
  let events=[];
  try{events=forecastEvents(i)}catch(err){console.error('Prévisions indisponibles',err);events=[]}
...
function diagnostic(){
  ...
  return layout(`<div class="section-title diagnostic-title"><div><h2>Diagnostic KerBudget</h2><small>Contrôle de l’application et de tes données locales.</small></div><button class="btn secondary" data-go="more">Retour</button></div>
    ...
    <section class="diagnostic-grid">
      <div><span>Version</span><strong>3.7.7 Test</strong></div><div><span>Années disponibles</span><strong>${availableYears().length}</strong></div><div><span>Taille des données</span><strong>${snap.size}</strong></div>
...
const VERSION_HISTORY=[
  {version:'3.7.7 Test',date:'6 août 2026',items:['Numéro de version harmonisé dans toute l’application.','Cache renouvelé pour afficher immédiatement la bonne version.','Stabilisation du module Prêts conservée.']},
  {version:'3.7.6 Test',date:'6 août 2026',items:['Échéanciers de prêts fiabilisés et triés automatiquement.','Pointage protégé contre les doublons et les identifiants anciens.','Capital restant recalculé après modification d’une mensualité.']},
...
function startKerBudget(){
  view='home';
  document.querySelectorAll('.bottom button').forEach(b=>b.onclick=()=>{view=b.dataset.view;render()});
  const modal=document.querySelector('#modal');
  if(modal)modal.onclick=e=>{if(e.target.id==='modal')closeModal()};
  if(selectedYear===new Date().getFullYear())ensureRecurringForMonth(month);
  render();
  // Certains navigateurs Android restaurent la structure avant que le premier rendu soit peint.
  // Un second contrôle au cycle d'affichage garantit que l'accueil n'est jamais laissé vide.
  requestAnimationFrame(()=>{
    const app=document.querySelector('#app');
    if(app&&!app.firstElementChild){view='home';render()}
  });
  maybeOfferNewYear();
}
window.addEventListener('beforeinstallprompt',e=>{e.preventDefault();deferredPrompt=e;let b=document.querySelector('#installBtn');if(b){b.hidden=false;b.onclick=()=>deferredPrompt.prompt()}});
window.addEventListener('pageshow',()=>{const app=document.querySelector('#app');if(app&&!app.firstElementChild){view='home';render()}});
if('serviceWorker'in navigator){navigator.serviceWorker.register('sw.js?v=374').then(r=>r.update()).catch(()=>{});}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',startKerBudget,{once:true});else startKerBudget();
