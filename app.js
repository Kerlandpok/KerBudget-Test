...
const D=window.INITIAL_DATA, KEY='budget2026.test.v2', BACKUP_KEY='budget2026.test.backups', APP_VERSION='3.0.0-test', MAX_BACKUPS=12;
...
function home(){const current=balance(month,true),projected=balance(month,false),savings=savingsBalance(),pending=mtx(month).filter(t=>!t.pointed),recurringPending=recurringToGenerate(month),incomplete=incompleteMovements(month),upcoming=importantUpcoming(month),alerts=budgetAlerts(month),t=totals(month),p=planned(month),available=Math.max(0,p.expense-t.expense),budgetUsed=p.expense>0?Math.min(100,Math.max(0,t.expense/p.expense*100)):0,saved=monthSavings(month),actions=[];if(pending.length)actions.push({label:`Pointer ${pending.length} opération${pending.length>1?'s':''}`,go:'transactions'});if(recurringPending.length)actions.push({label:`Générer ${recurringPending.length} facture${recurringPending.length>1?'s':''}`,go:'recurring'});if(incomplete.length)actions.push({label:`Compléter ${incomplete.length} mouvement${incomplete.length>1?'s':''}`,go:'transactions'});return layout(`${monthbar()}<section class="cockpit-balance revised"><div class="cockpit-main available-main"><span>Disponible jusqu’à la fin du mois</span><strong>${euro(available)}</strong><small>Budget consommé : ${budgetUsed.toFixed(0)} %</small><div class="progress large"><i style="width:${budgetUsed}%"></i></div></div><div class="cockpit-side"><div><span>Compte courant</span><b>${euro(current)}</b><small>Prévu : ${euro(projected)}</small></div><div><span>Épargne</span><b>${euro(savings)}</b><small>Ce mois : ${euro(saved)}</small></div></div></section>${actions.length?`<section class="cockpit-block"><div class="cockpit-title"><h2>À faire</h2><span>${actions.length}</span></div><div class="action-list">${actions.map(a=>`<button data-go="${a.go}"><span>→</span>${esc(a.label)}</button>`).join('')}</div></section>`:`<section class="cockpit-ok"><span>✓</span><div><strong>Tout est à jour</strong><small>Aucune action nécessaire.</small></div></section>`}${upcoming.length?`<section class="cockpit-block"><div class="cockpit-title"><h2>À venir</h2><button class="text-link" data-go="transactions">Tout voir</button></div><div class="upcoming-list">${upcoming.map(x=>`<div class="upcoming-item" ${x.source==='movement'?`data-edit="${esc(x.id)}"`:''}><div class="upcoming-date">${String(x.day).padStart(2,'0')}</div><div class="upcoming-copy"><strong>${esc(x.title)}</strong><small>Solde après : ${euro(x.balanceAfter)}</small></div><b class="${x.amount>=0?'positive':'negative'}">${x.amount>=0?'+':'−'}${euro(Math.abs(x.amount))}</b></div>`).join('')}</div></section>`:''}<section class="cockpit-block"><div class="cockpit-title"><h2>Situation du mois</h2><button class="text-link" data-go="report">Bilan</button></div><div class="situation-grid"><div><span>Revenus</span><b class="positive">${euro(t.income)}</b></div><div><span>Dépenses</span><b class="negative">${euro(t.expense)}</b></div><div><span>Épargne</span><b>${euro(saved)}</b></div><div><span>Budget consommé</span><b>${budgetUsed.toFixed(0)} %</b></div></div></section>${alerts.length?`<section class="cockpit-block"><div class="cockpit-title"><h2>Alertes</h2></div><div class="alert-list">${alerts.map(a=>`<div class="alert-item ${a.tone}">${esc(a.text)}</div>`).join('')}</div></section>`:''}<button class="fab" data-add>+</button>`)}
...
function pointageRow(t){
  ...
  return `<div class="pointage-row ${t.pointed?'is-pointed':'is-pending'}" data-edit="${esc(t.id)}">
    <button class="point-toggle" data-toggle-point="${esc(t.id)}" aria-label="${t.pointed?'Marquer non pointé':'Pointer cette opération'}">${t.pointed?'✓':''}</button>
    <div class="pointage-copy"><div class="title">${esc(t.description||t.subcategory||'Mouvement')}</div><div class="sub">${t.day?String(t.day).padStart(2,'0')+'/':''}${String(t.month+1).padStart(2,'0')}/2026 · ${esc(typeLabel(t.type))}${flow}${t.subcategory?' · '+esc(t.subcategory):''}</div></div>
    <div class="amount ${positive?'positive':'negative'}">${positive?'+':'−'}${euro(Math.abs(t.amount))}</div>
  </div>`;
}
function filteredPointageRows(){
  return mtx(month).filter(t=>{
    ...
    const hay=(t.description+' '+t.category+' '+t.subcategory+' '+typeLabel(inferMovementType(t))).toLowerCase();
...
function pointageListHtml(){
  ...
  return rows.length?`<div class="pointage-list">${rows.map(pointageRow).join('')}</div>`:'<div class="empty pointage-empty">Aucune opération ne correspond à ces filtres.</div>';
}
function transactions(){
  ...
  const pointedAmount=pointed.reduce((s,t)=>s+(Number(t.amount)||0),0);
  const statusButtons=[['all','Tous',all.length],['pending','À pointer',pending.length],['pointed','Pointés',pointed.length]];
  const typeButtons=[['all','Toutes'],['income','Revenus'],['bill','Factures'],['expense','Dépenses'],['transfer','Épargne / virements']];
  return layout(`${monthbar()}
    <div class="section-title pointage-title"><div><h2>Pointage • ${state.months[month].month}</h2><small>Un seul espace pour retrouver, vérifier et pointer les opérations.</small></div><button class="btn" data-add>+ Ajouter</button></div>
    <section class="pointage-summary">
      <div class="summary-pending"><span>À pointer</span><strong>${pending.length}</strong><small>${euro(pendingAmount)}</small></div>
      <div><span>Pointées</span><strong>${pointed.length}</strong><small>${euro(pointedAmount)}</small></div>
      <div><span>Solde pointé</span><strong class="${balance(month,true)>=0?'positive':'negative'}">${euro(balance(month,true))}</strong><small>Solde bancaire suivi</small></div>
    </section>
    <section class="pointage-controls">
      ...
      <input class="search" id="search" value="${esc(txSearch)}" placeholder="Rechercher une opération…">
