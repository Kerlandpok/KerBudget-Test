...
const D=window.INITIAL_DATA, KEY='budget2026.test.v2', BACKUP_KEY='budget2026.test.backups', APP_VERSION='3.3.0-test', MAX_BACKUPS=12;
...
function smartInsights(i){
  const out=[], pending=unpointedMovements(i), week=upcomingWithinDays(i,7), f=forecastData(i), p=planned(i), t=totals(i);
  const weekExpenses=week.filter(x=>x.amount<0), weekTotal=weekExpenses.reduce((s,x)=>s+Math.abs(x.amount),0);
  if(pending.length)out.push({tone:'warning',icon:'!',text:`${pending.length} écriture${pending.length>1?'s':''} à pointer pour ${euro(pending.reduce((s,x)=>s+(Number(x.amount)||0),0))}.`,go:'transactions'});
  if(weekExpenses.length)out.push({tone:weekTotal>500?'warning':'info',icon:'7j',text:`${weekExpenses.length} sortie${weekExpenses.length>1?'s':''} prévue${weekExpenses.length>1?'s':''} dans les 7 jours, pour ${euro(weekTotal)}.`,go:'forecast'});
  ...
  else if(p.expense>0){const used=t.expense/p.expense*100;if(used>=90)out.push({tone:'warning',icon:'%',text:`${used.toFixed(0)} % du budget dépenses est déjà consommé.`,go:'budget'});else out.push({tone:'success',icon:'✓',text:`Le budget dépenses est consommé à ${used.toFixed(0)} %.`,go:'budget'});}
  return out.slice(0,3)
}
function home(){
  const current=balance(month,true), f=forecastData(month), savings=savingsBalance(), pending=unpointedMovements(month), week=upcomingWithinDays(month,7), weekOut=week.filter(x=>x.amount<0).reduce((s,x)=>s+Math.abs(x.amount),0), upcoming=f.rows.slice(0,5), insights=smartInsights(month), t=totals(month), p=planned(month), budgetUsed=p.expense>0?Math.min(100,Math.max(0,t.expense/p.expense*100)):0;
  return layout(`${monthbar()}
    ...
    <section class="today-grid">
      ...
      <button class="today-card savings" data-go="savings"><span class="today-icon">◆</span><small>Épargne totale</small><strong>${euro(savings)}</strong><em>Virements pointés</em></button>
    </section>
...
function transactions(){
  ...
  const statusButtons=[['all','Toutes',all.length],['pending','À pointer',pending.length],['pointed','Pointées',pointed.length]];
  const typeButtons=[['all','Tous types'],['income','Revenus'],['bill','Factures'],['expense','Dépenses'],['transfer','Épargne / virements']];
  return layout(`${monthbar()}
    <div class="section-title movements-title"><div><h2>Mouvements • ${state.months[month].month}</h2><small>Retrouve, pointe et modifie tes opérations au même endroit.</small></div><button class="btn" data-add>+ Ajouter</button></div>
